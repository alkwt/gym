...
function WorkoutSelector({ routines }) {
    ...
    const [allRoutines, setAllRoutines] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(routines);
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "WorkoutSelector.useEffect": ()=>{
            ...
            const deletedPresetsStr = localStorage.getItem("deletedPresetWorkouts");
            const deletedPresetIds = deletedPresetsStr ? JSON.parse(deletedPresetsStr) : [];
            const visiblePresetRoutines = routines.filter({
                "WorkoutSelector.useEffect.visiblePresetRoutines": (r)=>!deletedPresetIds.includes(r.id)
            }["WorkoutSelector.useEffect.visiblePresetRoutines"]);
            const customWorkoutsStr = localStorage.getItem("customWorkouts");
            const customWorkouts = customWorkoutsStr ? JSON.parse(customWorkoutsStr) : [];
            const combinedRoutines = [
                ...visiblePresetRoutines,
                ...customWorkouts
            ];
            const workoutOrderStr = localStorage.getItem("workoutOrder");
            if (workoutOrderStr) {
                try {
                    const workoutOrder = JSON.parse(workoutOrderStr);
                    const routineMap = new Map(combinedRoutines.map({
                        "WorkoutSelector.useEffect": (r)=>[
                                r.id,
                                r
                            ]
                    }["WorkoutSelector.useEffect"]));
                    const sortedRoutines = workoutOrder.map({
                        "WorkoutSelector.useEffect.sortedRoutines": (id)=>routineMap.get(id)
                    }["WorkoutSelector.useEffect.sortedRoutines"]).filter(Boolean);
                    const unsortedRoutines = combinedRoutines.filter({
                        "WorkoutSelector.useEffect.unsortedRoutines": (r)=>!workoutOrder.includes(r.id)
                    }["WorkoutSelector.useEffect.unsortedRoutines"]);
                    setAllRoutines([
                        ...sortedRoutines,
                        ...unsortedRoutines
                    ]);
                } catch (e) {
                    console.error("Failed to parse workout order from localStorage", e);
                    setAllRoutines(combinedRoutines);
                }
            } else {
                setAllRoutines(combinedRoutines);
            }
    ...
    const saveOrder = (routinesToSave)=>{
        const order = routinesToSave.map((r)=>r.id);
        localStorage.setItem("workoutOrder", JSON.stringify(order));
    };
    const handleDeleteWorkout = ()=>{
        if (!routineToDelete) return;
        if (routineToDelete.id.startsWith('custom-')) {
            const customWorkoutsStr = localStorage.getItem("customWorkouts");
            if (customWorkoutsStr) {
                const customWorkouts = JSON.parse(customWorkoutsStr);
                const updatedWorkouts = customWorkouts.filter((w)=>w.id !== routineToDelete.id);
                localStorage.setItem("customWorkouts", JSON.stringify(updatedWorkouts));
            }
        } else {
            const deletedPresetsStr = localStorage.getItem("deletedPresetWorkouts");
            const deletedPresetIds = deletedPresetsStr ? JSON.parse(deletedPresetsStr) : [];
            if (!deletedPresetIds.includes(routineToDelete.id)) {
                deletedPresetIds.push(routineToDelete.id);
                localStorage.setItem("deletedPresetWorkouts", JSON.stringify(deletedPresetIds));
            }
        }
        const updatedAllRoutines = allRoutines.filter((r)=>r.id !== routineToDelete.id);
        setAllRoutines(updatedAllRoutines);
        saveOrder(updatedAllRoutines);
...
_s(WorkoutSelector, "6zDYErCm0ozV2m3m44csSxFh1kI=", false, function() {
    return [
...
//# sourceMappingURL=src_components_db5c5a1b._.js.map